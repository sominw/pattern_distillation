    ...
    for idx, row in data.iterrows(): 
        ...
        if not together_api:
            if data_name == 'alpaca':
                ...
                if row[input_col] == np.nan or row[input_col] == 'nan':
                    processed_row = {
                        "id": row[id_col], 
                        "text": row[text_col],
            ...
            if data_name == 'alpaca': 
                ...
                if row[input_col] == np.nan:
                    processed_row = {
                        "id": row[id_col], 
                        "text": row[text_col],
                        # "gold_summary": row[summary_col], 
                        "generated_summary": call_together_api(PROMPT, str(row[text_col]), model_id, key, client)
                    }
                else: 
                    if 'nan' in str(row[input_col]):
                        processed_row = {
                            "id": row[id_col], 
                            "text": row[text_col],
                            # "gold_summary": row[summary_col], 
